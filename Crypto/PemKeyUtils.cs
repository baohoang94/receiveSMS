...
namespace Crypto
{
    public class PemKeyUtils
    {
        ...
        public static RSACryptoServiceProvider GetRSAProviderFromPemFile(string pemPathFile)
        {
            string pemstr = File.ReadAllText(pemPathFile).Trim();
            return GetRSAProviderFromPemString(pemstr);
        }

        public static RSACryptoServiceProvider GetRSAProviderFromPemString(string pemTextPlain)
        {
            pemTextPlain = pemTextPlain.Trim();
            bool isPrivateKeyFile = true;
            if (pemTextPlain.StartsWith(pempubheader) && pemTextPlain.EndsWith(pempubfooter))
                isPrivateKeyFile = false;

            byte[] pemkey;
            if (isPrivateKeyFile)
                pemkey = DecodeOpenSSLPrivateKey(pemTextPlain);
            else
                pemkey = DecodeOpenSSLPublicKey(pemTextPlain);
